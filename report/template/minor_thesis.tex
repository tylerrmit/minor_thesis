...
The aim of this research project was to investigate whether it is possible to construct a dataset or map of bicycle lanes in a local area, by collecting street scene images at known coordinates, and then using a ``deep learning'' machine learning model to detect locations where bicycle lane markings are found.  If a baseline map of bicycle lanes can be built in this way, then the process could be extended in future to gather information about other significant factors, such as how frequently the bicycle lane is obstructed by parked vehicles, or the presence of debris or damage to the road surface.
...
Images were accepted for inclusion in the dataset if the bicycle lane marking was clear and unambiguous.  If a human could tell that a while marking was a bicycle lane marking, but only based on where it appeared in the image relative to other objects, then it was not included.

The GUI typically allowed multiple candidate intersections to be assessed per minute.  An initial set of 256 images was collected over the course of approximately 4 hours, based on a random sample of points across the State of Victoria.
...
It was discovered that some intersections would be missing on roads the margins of the survey area if the intersecting road existed entirely outside the bounds of the OpenStreetMap XML extract for the survey are.  A second extract, capturing a margin of at least 200 metres around the survey area, was used to rectify the missing intersection issue.  Roads in the second extract were not considered part of the survey, but data about them was taken into account to ensure that intersections at the margin of the survey area were not missed.
...
\subsection{Correcting dash camera imagesfor lens distortion}
\label{s:rq4a}
...
An initial dataset of 256 Google Street View images was gathered, with the option to gather more, if necessary, to satisfy either of the first two research questions.  They were labelled and split into 205 ``training''  images and 51 ``test'' images, according to an 80:20 split.  Five pre-trained object detection models from the TensorFlow 2 Model Garden were trained and evaluated using these ``training'' and ``test'' datasets, through a process of transfer learning.
...
The ``Faster R-CNN ResNet50 V1 640x640'' model was able to successfully locate a bicycle lane marking in \textit{every single image in ``training'' and ``test'' datasets where one appeared}.  It was not fooled by a single image with \textit{no bicycle lane markings} that had been included in the ``training'' dataset.
...
The initial dataset of 256 images appeared to be sufficient to get reliable detection results with this model, so no further images were collected from Google Street View for the ``training'' and ``test'' datasets.
...
Table \ref{table_metres_rq2_me} shows a comparison between the routes detected for Mount Eliza from the Google Street View images to what is recorded in OpenStreetMap, in terms of metres.  The two sources agreed upon 2,344 metres of bicycle lane routes, however the detection model found an additional 11,817 metres that were not recorded in OpenStreetMap.

Based on an in-person survey of the area, and inspection of the Google Street View images that were downloaded, the detection model correctly detected all bicycle lanes within the survey area, with no false positives, and it identified two routes that had not been recorded in OpenStreetMap.
 

\subsection{Heidelberg}

After surveying Mount Eliza with both the Google Street View and dash camera approaches, the Google Street View approach was tested on an sample area in Heidelberg, Victoria.  The purpose of this exercise was to assess how well the detection models generalised to a more heavily built-up area, closer to the city.  The specific area was chosen due to the potential availability of a contact to help survey routes in dispute in person, if necessary.  It included both residential and commercial streets.

The initial results from running the model that was trained only on Google Street View images were poor, due to an overwhelming volume of false-positive detections.  Figure \ref{heidelberg_bad} shows a comparison between the map generated from the detection model and OpenStreetMap.  The routes marked in blue represent the many streets where the detection model assumed there was a bicycle lane route that is perhaps not really there, according to OpenStreetMap.

\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{h_map1_4.png}
\caption{Comparison between detection model and OpenStreetMap for Heidelberg, model trained on GSV images only}
\label{fig:heidelberg_bad}
\end{figure}

Reviewing the images for false positives, it was found that a large number of them were caused by white markings on the road, and other distractions that had been dealt with when creating a larger dataset to train the dash camera model used to survey Mount Eliza for research question 3.  Therefore, the more advanced model that had been trained to ignore many common white road markings via example dash camera images was used to re-process the Google Street View data for Heidelberg.  The updated results are show in in figure \ref{fig:heidelberg_good}.

The green lines represent routes where both the detection model and OpenStreetMap agreed that a bicycle lane route is present.  The red lines represent routes that are recorded in OpenStreetMap but not detected, though the ones outside the bounding box describing the survey area can be ignored.  Google Street View images for the two ``red'' routes that run north-to-south along Cape Street and Stradbroke Street were examined, and it was found that there was no evidence of a bicycle lane in the images for either street.  The images may be out of date -- the Cape Street images were captured in May 2019, and the Stratbroke Street images were captured in March 2021 -- however the streets appeared too narrow to accommodate the addition of a bicycle lane without removing on-street parking.  These routes might be errors in the OpenStreetMap data.
...
There is a very short segment where OpenStreetMap records a cycleway on Lower Heidelberg Road near the intersection of Banksia Street.  This route would be barely larger than the nearby intersection, and is likely to be an error in OpenStreetMap.  The detection model marked some routes nearby, to the north and south of Banksia Street on Lower Heidelberg Road.  These were investigated and found to be caused by false-positive detection of large "Keep Clear" signs written on the road.  Keep clear signs are a known source of false positives that were added to the training dataset and labelled for the dash camera model.  However there were not very many examples in the training dataset.  Perhaps many of the false positives could be prevented by collecting and labelling more training data.
...
Due to COVID-19 restrictions \cite{lockdown_record} \cite{lockdown_5km}, dash camera footage could only be gathered in a circular area with a 15km radius, much of which was a body of water.  Therefore, the range of areas available to gather dash camera footage for research question 3 was restricted to a handful of outer metropolitan suburbs.  Footage from the Frankston, Langwarrin and Baxter area was used to train a the detection model to work with dash camera footage, and then footage from Mount Eliza was used to test it.
...
Because the few issues at 8,000 training steps exclusively involved challenging Google Street View images, and research question 3 involved applying a model to dash camera footage where no such issues were detected, and many more frames would be available to compensate for any missed detections, training was terminated after 8,000 steps.  The model was then used to conduct a survey of dash camera footage in the testing area of Mount Eliza.
...
It was found that, when processing an individual frame, the proposed method was vulnerable to ``noise'' from objects just outside the paved area, as discussed in section \ref{s:rq4c}.  The Canny/Hough approach to lane detection is typically applied to detecting the camera vehicle's own lane, and it can therefore limit the effect of off-road features by applying a mask that obscures anything not in the triangle directly in front of the vehicle.  If the mask includes everything immediately in front of the vehicle, but is too restrictive to see the lane markings, then that may not be an issue:  The vehicle is within its lane, even if it can't see the lines.  In this research project, the Canny/Hough approach was used to try to detect the next ``lane'' over, after detecting the camera vehicle's own lane.  It cannot be as aggressive at masking of areas that might or might not be off-road.

To limit the impact of these issues, the model grouped the frames for each road segment, from intersection to intersection, and assessed them collectively, as described in section \ref{s:rq4c}.  Summary metrics were calculated for each group of frames, to assess how frequently a paved shoulder was detected along the road segment, how wide any detected paved shoulder appeared to be on average, and how stable the lines were from frame to frame -- did the detected lines jump around a lot?

The process produced a CSV with one record per observed frame, reverse-geocoded using OSM data to give the name of the road for each point.  The summary statistics for each road segment were included as additional columns in the CSV.  The footage for the training area was reviewed to add a ``ground truth'' column, indicating whether a paved shoulder existed for the road segment or not.  A manual search was conducted to find thresholds for the summary statistics that generally gave a correct classification for each road segment in the training area.  The model was then applied with these thresholds to the test area of Mount Eliza.
...
The map gives relatively solid results on Humphries Road and the Nepean Highway, where there is a bicycle lane, a special case of a paved shoulder.  Part of the Nepean Highway was not detected, where the lane markings have faded and the ``noise'' from the foliage dragged the average lines for the camera vehicle too far to the left.

The other lines drawn on the map represent true paved shoulders, but they were easily interrupted by ``noise'' from the side of the road, or parked cars.  Many of the interruptions in the paved shoulder route on Old Mornington Road, to the west of the the Nepean Highway, were largely attributable to parked cars.
...
The use of Google Street View images to map bicycle lane routes in Mount Eliza was effective, and added value, finding routes that had not already been recorded in either the official ``Principal Bicycle Network'' dataset or the crowdsourced OpenStreetMap database.

\remark{COMMENT ON HEIDELBERG}
...
The model that was applied to the problem of detecting paved shoulders had significant limitations, and it was intended as a ``proof of concept'' to show that the general technique used to map bicycle lanes from the bicycle lane markings could be applied to other problems where information can be collected visually.

The model relied on explicit programming of an algorithm, and manually selected thresholds applied to summary statistics, based on observations in a training area.  It did not attempt to apply any of the more recent advances in machine learning or deep learning, where the model ``learns'' to solve the problem for itself from the data.  It was successful at creating an approximate map, without false positives, but the routes that were calculated were easily interrupted by a sufficient volume of parked cars or roadside ``noise''.
